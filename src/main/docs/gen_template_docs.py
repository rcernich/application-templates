...
TEMPLATE_DIR = attributes["templatedir"]
FILESETS = elements["fileset"]
DEST_DIR = attributes["destdir"]
...
def generate_templates():
    for fileset in FILESETS:
        baseDir = fileset.getDir().getAbsolutePath()
        for file in fileset:
            baseName = file.getName()
            sourceName = baseDir + "/" + baseName
            outTuple = baseName.rpartition(".")
            outName = DEST_DIR + "/" + outTuple[0] + "." + EXTENSION
            outDir = os.path.dirname(outName)

            if not os.path.exists(outDir):
                System.out.println("Creating output directories: " + outDir)
                os.makedirs(outDir)

            System.out.println("Generating docs for: " + sourceName)
            with open(sourceName) as data_file:
                data = json.load(data_file)
                
                # Presently we can't generate documentation for the 'secrets' templates
                if not 'labels' in data:
                    System.err.println("no labels for template %s, can't generate documentation\n"%\
                        (directory + "/" + template))
                    continue
